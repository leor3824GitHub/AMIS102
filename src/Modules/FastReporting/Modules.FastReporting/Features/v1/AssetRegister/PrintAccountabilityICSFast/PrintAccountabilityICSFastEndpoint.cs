using AMIS.Framework.Shared.Identity.Authorization;
using Mediator;
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Routing;

namespace AMIS.Modules.FastReporting.Features.v1.AssetRegister.PrintAccountabilityICSFast;

public static class PrintAccountabilityICSFastEndpoint
{
    private const string AccountabilityView = "Permissions.AssetRegister.Accountability.View";

    public static RouteHandlerBuilder Map(this IEndpointRouteBuilder endpoints) =>
        endpoints.MapGet("/{id:guid}/print", PrintFast)
            .WithName("FastReporting_PrintAccountabilityICS")
            .WithSummary("Generate a FastReport PDF for an ICS (SE_ICS accountability document)")
            .Produces(StatusCodes.Status200OK, null, "application/pdf")
            .Produces(StatusCodes.Status404NotFound)
            .RequirePermission(AccountabilityView);

    // Query: ?pageWidth=longbond|a4|legal   (default longbond)
    //        ?orientation=portrait|landscape (default portrait)
    //        ?minRows=1..40                  (default 15)
    private static async Task<IResult> PrintFast(
        Guid id,
        IMediator mediator,
        CancellationToken ct,
        string? pageWidth = null,
        string? orientation = null,
        int? minRows = null)
    {
        var paperSize = (pageWidth ?? "longbond").ToLowerInvariant();
        var orient = (orientation ?? "portrait").ToLowerInvariant() == "landscape"
            ? "landscape"
            : "portrait";
        var rows = Math.Clamp(minRows ?? 15, 1, 40);

        var dto = await mediator.Send(
            new PrintAccountabilityICSFastQuery(id, paperSize, orient, rows), ct);

        return Results.File(dto.Content, dto.ContentType, dto.FileName);
    }
}
