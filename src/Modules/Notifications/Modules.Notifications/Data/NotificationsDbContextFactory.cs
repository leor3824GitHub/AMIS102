...
/// <summary>
/// Design-time factory so EF tooling can build <see cref="NotificationsDbContext"/> for the FIRST migration
/// (no snapshot yet, so the context is discovered through this factory).
/// </summary>
public sealed class NotificationsDbContextFactory : IDesignTimeDbContextFactory<NotificationsDbContext>
{
    public NotificationsDbContext CreateDbContext(string[] args)
    {
        ...
        {
            throw new NotSupportedException($"Database provider '{provider}' is not supported for NotificationsDbContext migrations.");
        }
    ...
    private sealed class DesignTimeHostEnvironment : IHostEnvironment
    {
        ...
        public string ApplicationName { get; set; } = nameof(NotificationsDbContextFactory);
