    ...
    public async Task SeedAsync(CancellationToken cancellationToken)
    {
        ...
        if (!await context.Suppliers.AnyAsync(cancellationToken).ConfigureAwait(false))
        {
            var suppliers = new[]
            {
                ...
                Supplier.Create("SUP-009", "PowerCell Energy", "901-234-567-008", "NON-VAT", "Batteries, UPS, and power supplies", "Lena Uy", "lena@powercell.example", "+63-2-8800-1009", "7 Power Drive, Para�aque City"),

                // Furniture & fixtures
                Supplier.Create("SUP-010", "Furnishings Co.", "112-233-445-009", "NON-VAT", "Office furniture and ergonomic chairs", "Eve Turner", "eve@furnishings.example", "+63-2-8800-1010", "99 Furniture Rd, Las Pi�as City"),
        ...
        if (!await context.Employees.AnyAsync(cancellationToken).ConfigureAwait(false))
        {
            ...
            var employees = new[]
            {
                ...
                // IT
                EmployeeProfile.Create("EMP-010", "Roel",      "Caperig",   Off("00B"),     Dep("AGS"),  Pos("POS-SR-ENG"), null, "admin@root.com"),
        ...
        // Abstract of Canvass — seeded separately so it also runs on existing deployments
        if (!await context.ReportSignatories.AnyAsync(s => s.ReportType == "AbstractOfCanvass", cancellationToken).ConfigureAwait(false))
        {
            ...
            {
                // Abstract of Canvass — BOPC/ROPC Committee
