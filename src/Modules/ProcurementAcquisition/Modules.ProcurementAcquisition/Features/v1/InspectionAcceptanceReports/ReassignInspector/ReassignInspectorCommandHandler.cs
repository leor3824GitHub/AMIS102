...
public sealed class ReassignInspectorCommandHandler(
    ProcurementDbContext dbContext) : ICommandHandler<ReassignInspectorCommand, InspectionAcceptanceReportDto>
{
    public async ValueTask<InspectionAcceptanceReportDto> Handle(ReassignInspectorCommand command, CancellationToken cancellationToken)
    {
