...
public sealed class ExpandLineByQuantityCommandHandler(
    ProcurementDbContext dbContext) : ICommandHandler<ExpandLineByQuantityCommand, InspectionAcceptanceReportDto>
{
    public async ValueTask<InspectionAcceptanceReportDto> Handle(ExpandLineByQuantityCommand command, CancellationToken cancellationToken)
    {
