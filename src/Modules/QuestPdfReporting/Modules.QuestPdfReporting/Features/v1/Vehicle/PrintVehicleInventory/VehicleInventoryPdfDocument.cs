...
internal sealed class VehicleInventoryPdfDocument(
    ...
    string                             orientation = "landscape") : IDocument
{
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Column(col =>
        {
            ...
            if (org is not null)
            {
                col.Item().AlignCenter().Text(org.Name.ToUpperInvariant()).Bold().FontSize(11);
                if (!string.IsNullOrWhiteSpace(org.Address))
                    col.Item().AlignCenter().Text(org.Address).FontSize(8);
            }
            col.Item().PaddingTop(6).AlignCenter().Text("INVENTORY OF MOTOR VEHICLES").Bold().FontSize(11);
            col.Item().AlignCenter()
                .Text($"As of {(asOfDate ?? DateTime.Today):MMMM d, yyyy}").FontSize(9);
    ...
    private void ComposeBody(IContainer container)
    {
        container.PaddingTop(4).Table(table =>
        {
            ...
            foreach (var v in items)
            {
                ...
                table.Cell().Border(0.5f).Padding(2).Text(BuildDescription(v)).FontSize(7);
                ...
                table.Cell().Border(0.5f).Padding(2).Text(BuildOfficer(v)).FontSize(7);
            }
        });
    }
