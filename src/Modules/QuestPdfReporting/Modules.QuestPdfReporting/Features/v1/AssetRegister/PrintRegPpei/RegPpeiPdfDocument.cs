...
/// (COA Annex A.4) registry. One sheet per Fund Cluster × PPE classification; each row is one
/// transaction (issue / return / re-issue / disposal) filling exactly one of the form's column
/// groups, with a running Balance column.
/// </summary>
internal sealed class RegPpeiPdfDocument(
    RegPpeiReportDto         report,
    ...
    float                    marginMm    = 12f) : IDocument
{
    ...
    private void ComposeHeader(IContainer container)
    {
        ...
        {
            col.Item().AlignCenter().Text("REGISTRY OF PROPERTY, PLANT AND EQUIPMENT ISSUED").Bold().FontSize(11);
        ...
        container.PaddingTop(4).Column(col =>
        {
            if (report.TotalTransactions == 0)
            {
                col.Item().Border(0.5f).Padding(6).AlignCenter()
                    .Text("No property, plant and equipment transactions as of this date.").Italic().FontSize(8);
                return;
            }

            // One sheet per Fund Cluster × PPE classification — each sheet gets its own header block
            // (Entity Name / Fund Cluster / Property, Plant and Equipment / Sheet No.) and its own ledger
            // table, page-broken from the next.
            ...
            for (var si = 0; si < sheets.Count; si++)
            {
                ...
                col.Item().Row(row =>
                {
                    row.RelativeItem().Column(left =>
                    {
                        ...
                        left.Item().Text(t =>
                        {
                            t.Span("Property, Plant and Equipment: ").FontSize(8);
    ...
    private static void ComposeSheetTable(IContainer container, RegPpeiClassificationGroupDto sheet)
    {
        ...
        container.Table(table =>
        {
            table.ColumnsDefinition(c =>
            {
                ...
                c.RelativeColumn(2.3f);  //  2 PAR/RRP No.
                c.RelativeColumn(2.3f);  //  3 Property No.
            ...
            // Two-tier header: grouped columns (Reference / Issued / Returned / Re-issued / Disposed /
            // Balance) with Qty + Office/Officer sub-columns. Repeats on every page.
            table.Header(h =>
            {
                ...
                h.Cell().Element(HeadCell).Text("PAR/RRP No.").Style(headStyle);
                h.Cell().Element(HeadCell).Text("Property, Plant and Equipment No.").Style(headStyle);
