...
/// <summary>Report on Property Issued (RPI) — PPE issued to accountable officers via PAR.</summary>
internal sealed class RpiPdfDocument(
    RpiReportDto             report,
    ...
    float                    marginMm    = 12f) : IDocument
{
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Column(col =>
        {
            ...
            col.Item().PaddingTop(6).AlignCenter().Text("REPORT ON PROPERTY ISSUED").Bold().FontSize(11);
        ...
        container.PaddingTop(4).Column(col =>
        {
            col.Item().Table(table =>
            {
                table.ColumnsDefinition(c =>
                {
                    ...
                    c.ConstantColumn(58);    // Date Acquired
                });

                table.Header(h =>
                {
                    h.Cell().Border(1).Padding(2).AlignCenter().Text("No.").Style(style);
                    h.Cell().Border(1).Padding(2).AlignCenter().Text("PAR No.").Style(style);
                    ...
                    h.Cell().Border(1).Padding(2).AlignCenter().Text("EUL").Style(style);
                ...
                foreach (var r in report.Items)
                {
                    ...
                    table.Cell().Border(0.5f).Padding(2).AlignCenter().Text(r.EstimatedUsefulLifeYears.ToString()).FontSize(8);
                    table.Cell().Border(0.5f).Padding(2).AlignCenter().Text(r.DateAcquired.ToString("yyyy-MM-dd")).FontSize(8);
                    no++;
                }

                if (report.Items.Count == 0)
                    table.Cell().ColumnSpan(12).Border(0.5f).Padding(6).AlignCenter()
                        .Text("No property issued for this period.").Italic().FontSize(8);
            });
