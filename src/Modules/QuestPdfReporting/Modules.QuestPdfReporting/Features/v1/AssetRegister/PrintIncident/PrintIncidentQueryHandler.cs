...
{
    public async ValueTask<byte[]> Handle(PrintIncidentQuery query, CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(query);

        var report = await mediator.Send(new GetIncidentReportDocumentQuery(query.IncidentReportId), ct).ConfigureAwait(false)
