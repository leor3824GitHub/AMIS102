...
public sealed class AssetManagementDbContextFactory : IDesignTimeDbContextFactory<AssetManagementDbContext>
{
    public AssetManagementDbContext CreateDbContext(string[] args)
    {
        ...
        {
            throw new NotSupportedException($"Database provider '{provider}' is not supported for AssetManagementDbContext migrations.");
        }
    ...
    private sealed class DesignTimeHostEnvironment : IHostEnvironment
    {
        ...
        public string ApplicationName { get; set; } = nameof(AssetManagementDbContextFactory);
