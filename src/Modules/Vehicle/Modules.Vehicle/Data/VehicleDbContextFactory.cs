...
public class VehicleDbContextFactory : IDesignTimeDbContextFactory<VehicleDbContext>
{
    public VehicleDbContext CreateDbContext(string[] args)
    {
        ...
        {
            throw new NotSupportedException($"Database provider '{provider}' is not supported for VehicleDbContext migrations.");
        }
        ...
        return new VehicleDbContext(tenantAccessor, optionsBuilder.Options, databaseOptions, new DesignTimeHostEnvironment());
    }
    ...
    private sealed class DesignTimeHostEnvironment : IHostEnvironment
    {
        ...
        public string ApplicationName { get; set; } = nameof(VehicleDbContextFactory);
