...
{
    public async ValueTask<PropertyAccountabilityDto?> Handle(GetMyAccountabilityDetailQuery query, CancellationToken cancellationToken)
    {
        ...
        var dto = await mediator.Send(new GetAccountabilityQuery(query.Id), cancellationToken).ConfigureAwait(false);
