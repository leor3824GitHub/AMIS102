...
/// <summary>
/// Design-time factory so EF tooling can build <see cref="ChatDbContext"/> for the FIRST migration
/// (the context has no snapshot yet, so it is discovered through this factory). See CHAT-PORT-PLAN.md Phase 2.
/// </summary>
public sealed class ChatDbContextFactory : IDesignTimeDbContextFactory<ChatDbContext>
{
    public ChatDbContext CreateDbContext(string[] args)
    {
        ...
        {
            throw new NotSupportedException($"Database provider '{provider}' is not supported for ChatDbContext migrations.");
        }
    ...
    private sealed class DesignTimeHostEnvironment : IHostEnvironment
    {
        ...
        public string ApplicationName { get; set; } = nameof(ChatDbContextFactory);
