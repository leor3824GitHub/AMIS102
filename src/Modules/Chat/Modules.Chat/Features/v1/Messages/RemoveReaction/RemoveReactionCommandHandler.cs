...
public sealed class RemoveReactionCommandHandler : ICommandHandler<RemoveReactionCommand, MessageDto>
{
    private readonly ChatDbContext _dbContext;
    private readonly ICurrentUser _currentUser;

    public RemoveReactionCommandHandler(ChatDbContext dbContext, ICurrentUser currentUser)
    {
        _dbContext = dbContext;
        _currentUser = currentUser;
    }

    public async ValueTask<MessageDto> Handle(RemoveReactionCommand command, CancellationToken cancellationToken)
    {
        ...
        message.RemoveReaction(userId, command.Emoji.Trim());
