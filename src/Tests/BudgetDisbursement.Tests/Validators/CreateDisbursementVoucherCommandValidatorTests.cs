...
namespace BudgetDisbursement.Tests.Validators;
...
public sealed class CreateDisbursementVoucherCommandValidatorTests
{
    ...
    [Fact]
    public void Validate_EmptyBudgetUtilizationRequestId_Fails()
    {
        var command = ValidCommand() with { BudgetUtilizationRequestId = Guid.Empty };

        var result = _sut.Validate(command);

        result.IsValid.ShouldBeFalse();
        result.Errors.ShouldContain(e => e.PropertyName == nameof(command.BudgetUtilizationRequestId));
    }
