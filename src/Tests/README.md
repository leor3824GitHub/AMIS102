...
This folder contains solution-wide architecture tests for the AMIS (Asset Management Information System) .NET 10 Starter Kit. The goal is to automatically enforce layering, dependency, and naming rules as the codebase evolves.
...
- **Feature layering**: feature types under `Modules.*.Features.v{version}` are checked with NetArchTest to depend only on allowed layers (System/Microsoft, `AMIS.Framework.*`, their module, and module contracts).
- **Host boundaries**: module code must not depend on host projects (AMIS.Api / AMIS.Blazor / AMIS.AppHost), and hosts must not depend directly on module feature or data internals.
...
- Run all tests (including architecture tests): `dotnet test src/AMIS.Framework.slnx`.
