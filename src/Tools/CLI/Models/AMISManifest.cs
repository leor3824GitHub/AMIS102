...
/// <summary>
/// Manifest file stored in .AMIS/manifest.json to track project configuration and versions.
/// Used by the upgrade system to detect changes and apply updates.
/// </summary>
internal sealed class AMISManifest
{
    ...
    /// <summary>
    /// Version of the AMIS framework packages used.
    /// </summary>
    [JsonPropertyName("AMISVersion")]
    public required string AMISVersion { get; set; }
    ...
    public required AMISManifestOptions Options { get; set; }

    /// <summary>
    /// Version tracking for building blocks and customizations.
    /// </summary>
    [JsonPropertyName("tracking")]
    public required AMISManifestTracking Tracking { get; set; }
    ...
    /// </summary>
    public static AMISManifest? FromJson(string json) => JsonSerializer.Deserialize<AMISManifest>(json, JsonOptions);

    /// <summary>
    /// Try to load a manifest from a project directory.
    /// </summary>
    public static AMISManifest? TryLoad(string projectPath)
    {
        var manifestPath = Path.Combine(projectPath, ".AMIS", "manifest.json");
    ...
    {
        var AMISDir = Path.Combine(projectPath, ".AMIS");
        Directory.CreateDirectory(AMISDir);
        File.WriteAllText(Path.Combine(AMISDir, "manifest.json"), ToJson());
    }
}
...
/// </summary>
internal sealed class AMISManifestOptions
{
...
/// </summary>
internal sealed class AMISManifestTracking
{
    /// <summary>
