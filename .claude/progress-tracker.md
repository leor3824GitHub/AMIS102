...
- [x] **2nd pass — ReturnedProperty (RRSP/RRP) + report + signed-copy upload:** added `ReturnedPropertyReceiptLifecycleTests` (13 tests covering the Pending→Inspected→Accepted/Rejected/Cancelled state machine + guards; the aggregate previously had zero coverage). Verified report PDF + wet-signed `UploadSignedDocument` flow end-to-end — 2026-06-17
- [x] **ReturnedProperty — requester nominates the inspector:** the "Request a Return" form now requires selecting an **Inspector** (employee picker). Captured at request time as a new required `AssignedInspector` (`EmployeeRef`) on `ReturnedPropertyReceipt`, distinct from `InspectedBy` (who actually performs the inspection). Flows through command/validator/handler/DTO/mapper + EF owned-type config; migration `AssetRegister_ReturnedPropertyAssignedInspector`. Shown in the detail card and inspect dialog. Lifecycle tests assert capture — 2026-06-17
- [x] **ReturnedProperty — reassign the nominated inspector (mirrors IAR `ReassignInspector`):** new `ReassignReturnedPropertyInspectorCommand` + `ReturnedPropertyReceipt.ReassignInspector(EmployeeRef)` (allowed only while Pending), `POST /returned-property/{id}/reassign-inspector` gated by `ReturnedProperty.Create`, Blazor `ReturnedPropertyReassignInspectorDialog` + row action on Pending rows. No schema change (reuses the `AssignedInspector` owned columns). 2 domain tests added — 2026-06-17
- [x] **ReturnedProperty — only the assigned inspector may inspect (aligned to IAR pattern):** examined `ProcurementAcquisition.InspectionAcceptanceReports` (the canonical inspection feature) and AssetRegister's own `AcceptAccountability` ownership gate. Brought the inspect flow into line: the actor is now resolved **server-side** from the authenticated identity via `CurrentEmployeeResolver` (the `InspectReturnedPropertyReceiptCommand` no longer carries a client-supplied `InspectedBy` — closes a spoofing vector), the domain guard throws `UnauthorizedAccessException` which the handler translates to `ForbiddenException` (403, was a generic 400), and the list Inspect button is gated to the assigned inspector (`AssignedInspectorEmployeeId` on the summary DTO). Negative lifecycle test added — 2026-06-17
- [x] **2nd pass — RRP Net Book Value:** the report's NBV column was blank ("manual entry"). User clarified NBV comes from the depreciation ledger. Now frozen into `AssetSnapshot` at return time from `AssetRegistry.CarryingAmount` (UnitCost − accumulated depreciation − impairment; = UnitCost for SE). Single `Snapshot()` factory + shared owned-type config touched, so it propagates to all snapshot owners; additive migration `AssetRegister_SnapshotNetBookValue` (Snapshot_NetBookValue across 6 tables, default 0 backfill). Report fills per-row NBV + a NBV grand total — 2026-06-17
